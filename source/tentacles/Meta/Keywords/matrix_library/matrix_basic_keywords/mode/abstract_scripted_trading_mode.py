...
class AbstractScripted2TradingMode(abstract_trading_mode.AbstractTradingMode):
    ...
    async def reload_scripts(self, action: str or dict = None):
        for is_live in (False, True):
            if (is_live and self.__class__.TRADING_SCRIPT_MODULE) or (not is_live and self.__class__.BACKTESTING_SCRIPT_MODULE):
    ...
    async def ohlcv_callback(self, exchange: str, exchange_id: str, cryptocurrency: str, symbol: str,
        ...
        async with self.trading_mode_trigger(), self.trading_mode.remote_signal_publisher(symbol):
            ...
            await self.call_script(self.matrix_id, cryptocurrency, symbol, time_frame,
                                   ...
                                   trigger_time,
                                   candle=candle,
                                   action=matrix_enums.TradingModeCommands.INIT_CALL if init_call else matrix_enums.TradingModeCommands.OHLC_CALLBACK,
                                   init_call=init_call)
